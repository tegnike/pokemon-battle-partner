...
`field` は文字列のまま保持するが、値は常に構造化FieldState (`src/fieldState.ts`) の正規形 (`全体: 雨 / 相手側: ステルスロック / 自分側: リフレクター` 形式) に寄せる。新しい観測は全置換ではなく構造化マージで反映するため、「雨になった」という報告だけで設置物や壁の記録が消えることはない。天候は同時に1つで置き換え、「〜解除」「〜がやんだ」などの報告で該当効果だけを取り除く。壁・追い風・天候のダメージ/素早さ補正は `fieldState.ts` の単一パーサを参照する。
...
対戦状況による補正 (`adjustDamagePercentForBattleState`) は、能力ランク(攻撃/特攻)、相手側リフレクター・ひかりのかべ・オーロラベール(0.5倍)、天候(雨/晴れでみず・ほのお1.5倍/0.5倍)を反映し、補正内容を候補理由に注記する。壁と天候の判定は構造化FieldState (`src/fieldState.ts`) を参照する。
...
対戦中のローカル候補生成 (`localBattleCandidates`):

- 現在対面を見る
- `activeOwn` が空の場合は、直前の技・交代・選出先頭から場の自分ポケモンを推定する
- 自分の技ごとに簡易ダメ計
- 最も通る技を候補にする
- 場の自分ポケモンの変化技(おにび、ステルスロックなど)も、効果説明つきの補助技候補として最大2つ加える。相手が既に状態異常・タイプ的に無効・設置済みなどの場合は抑制する (`src/mastra/turnEvaluation.ts`)
...
対戦中は相手→自分方向のダメージも概算し、最終判断プロンプトへ渡す (`src/mastra/turnEvaluation.ts`)。

- 相手の判明技(なければタイプ一致90威力の仮定)を、攻撃最大振り想定でこちらの実数値に当てる
- こちらの特性・タイプによる無効(ふゆう、ひこう)を反映する
- 行動順の注記を付ける。トリックルーム中は「遅い方が先」として反転する
- 交代先候補(選出済み・ひんしでない控え)が同じ技を受け出しした場合の最大被弾も出し、データ無しの「受けられる」説明を防ぐ
...
- 場のポケモンが覚えている技であれば、候補一覧に無い技(補助技を含む)でも通す。ただし相手に無効(0倍)と分かる技は候補内の一手へ補正する
